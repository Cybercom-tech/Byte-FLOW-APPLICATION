...
function TeacherCourses() {
  ...
  const [courseAssignmentStatus, setCourseAssignmentStatus] = useState({}) // { courseId: { taughtBy: string } | null }
  ...
  useEffect(() => {
    ...
    const loadTeacherData = async () => {
      ...
      const teacherDocId = profile?._id || profile?.id
      
      // Update state immediately as data arrives (teacherDocId = teacher document _id for "taught by other" checks)
      setTeacherInfo({
        id: userId,
        name: teacherName,
        teacherDocId: teacherDocId ? String(teacherDocId) : null
      })
...
useEffect(() => {
  ...
  const handleSelectCourse = (courseId) => {
    // Don't allow selecting courses that are already taught by another teacher
    if (courseAssignmentStatus[String(courseId)]?.taughtBy) return
  ...
  // Load which courses are already taught by another teacher (for Select Existing tab)
  useEffect(() => {
    if (activeTab !== 'select' || !teacherInfo?.id) {
      if (activeTab !== 'select') setAssignmentStatusLoading(false)
      return
    }
    const createdIds = new Set(myCreatedCourses.map(c => String(c.id)))
    const allIds = new Map()
    coursesData.forEach(c => allIds.set(String(c.id), true))
    if (Array.isArray(allCourses)) allCourses.forEach(c => allIds.set(String(c.id), true))
    const courseIdsToCheck = Array.from(allIds.keys()).filter(id => !createdIds.has(id))
    if (courseIdsToCheck.length === 0) {
      setCourseAssignmentStatus({})
      setAssignmentStatusLoading(false)
      return
    }
    let cancelled = false
    setAssignmentStatusLoading(true)
    getCoursesAssignmentStatus(courseIdsToCheck)
      .then((status) => {
      ...
      .catch(() => { if (!cancelled) setCourseAssignmentStatus({}) })
      .finally(() => { if (!cancelled) setAssignmentStatusLoading(false) })
    return () => { cancelled = true }
  }, [activeTab, teacherInfo?.id, allCourses, myCreatedCourses])
          ...
          <div>
            ...
            {assignmentStatusLoading && (
              <p style={{ fontSize: '0.9rem', color: '#6c757d', marginBottom: '1rem' }}>
                ...
                Checking which courses are available to teach...
              </p>
            )}
            {/* Search */}
            <div style={{ marginBottom: '2rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {availableCourses.map(course => {
                ...
                // From API or fallback: course has another teacher (creator/owner) and it's not the current user
                const courseIdKey = String(course.id)
                const statusEntry = courseAssignmentStatus[courseIdKey] ?? courseAssignmentStatus[course.id]
                const statusTaughtBy = statusEntry && typeof statusEntry === 'object' && statusEntry.taughtBy ? statusEntry.taughtBy : null
                const courseHasOtherTeacher = course.teacherId && teacherInfo?.teacherDocId && String(course.teacherId) !== String(teacherInfo.teacherDocId)
                const taughtByOther = statusTaughtBy || (courseHasOtherTeacher ? (course.instructor || course.teacherName || course.teacher || 'Another instructor') : null)
                
                return (
                  <div
                    key={course.id}
                    style={{
                      backgroundColor: taughtByOther ? '#f8f9fa' : '#ffffff',
                      padding: '1.5rem',
                      borderRadius: '8px',
                      border: taughtByOther ? '1px solid #dee2e6' : '1px solid #e9ecef',
                      display: 'flex',
                      justifyContent: 'space-between',
                      alignItems: 'center',
                      gap: '1rem'
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {taughtByOther && (
                        <div style={{
                          ...
                          <i className="bi bi-info-circle" style={{ marginRight: '0.5rem' }}></i>
                          This course is already taught by someone{taughtByOther !== 'Another instructor' ? ` (${taughtByOther})` : ''}. Try creating your own by going to <button type="button" onClick={() => setActiveTab('create')} style={{ background: 'none', border: 'none', color: '#c85716', textDecoration: 'underline', cursor: 'pointer', fontWeight: '600' }}>Create New Course</button>.
                        </div>
                      )}
                    </div>
