...
function CourseDetail() {
  ...
  // Use actual reviews for rating display (works for default, admin, and teacher-created courses)
  const displayRating = courseReviews?.length > 0
    ? courseReviews.reduce((sum, r) => sum + (r.rating || 0), 0) / courseReviews.length
    : (course.rating ?? 0)
  const displayRatingCount = courseReviews?.length > 0 ? courseReviews.length : (course.ratingCount ?? 0)
  const hasRatings = displayRating > 0 || displayRatingCount > 0
            ...
            <div style={{ marginBottom: '2rem' }}>
              ...
              {hasRatings ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
                  <span style={{ fontWeight: '600', color: '#110a06' }}>{Math.round(displayRating * 10) / 10}</span>
